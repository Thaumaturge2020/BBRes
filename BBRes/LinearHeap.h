...
public:
	...
	void init_by_minus_three(ui _n, ui _key_cap, ui *_id_s, ui *_key_s, ui *_increase_val, ui minus_val,ui multi = 4) {
		if(key_s == nullptr) key_s = new ui[n];
		if(pre_s == nullptr) pre_s = new ui[n];
		if(next_s == nullptr) next_s = new ui[n];
		if(head_s == nullptr) head_s = new ui[key_cap+1];
		...
		for(ui i = 0;i < _n;i ++) {
			ui id = _id_s[i];
			ui key = (minus_val- _key_s[id])*multi + _increase_val[id];
			//assert(id < n); assert(key <= _key_cap);
	...
	bool pop_min(ui &id, ui &key) {// return true if success, return false otherwise
		...
		key_s[id] = key_cap+1;
		//assert(key_s[id] == key);
		// std::cerr << " ? convert " << next_s[7665] << std::endl;
