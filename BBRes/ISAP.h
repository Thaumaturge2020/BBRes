#ifndef _ISAP_SOLVER_
#define _ISAP_SOLVER_
...
class ISAPSolver{
    ...
    bool dijk(){
        ...
        while(Q->pop_min(x,key)){
            ...
            for(ui i = head[x];i;i = nex[i]){
                ui y = ver[i];
                // if(e[i])
                // std::cerr << "..." << h[x] + cost[i] - h[y] << std::endl;
                if(e[i] && dep[y] > dep[x] + h[x] + cost[i] - h[y]){
                    Q->decrement(y, dep[y] - (dep[x] + h[x] + cost[i] - h[y]));
                    dep[y] = dep[x] + h[x] + cost[i] - h[y];
                    // std::cerr << x << " " << y << " " << dep[y] << std::endl;
                }
                // std::cerr << "end!" << Q.size() << std::endl;
            }
        }
        for(ui i = 0;i < n;i ++) h[i] += dep[i];
        return dep[T] < INIT_LIMIT;
    }

    void inv_dijk(ui start){
        // std::cerr << "hey" << " " << n << " " << init_n*6+5 << std::endl;
        for(ui i = 0;i < n;i ++) vis[i] = 0,dep[i] = INIT_LIMIT;
        dep[start] = 0;
        while(!V.empty()) V.pop();
        ui x,key,bfs_cnt = 0;
        V.push(mp(dep[start],start));
        while(!V.empty()){
            // std::cerr << x << " " << std::endl;
            pii X = V.top();V.pop();x=X.se;
            if(vis[x]) continue;
            ++bfs_cnt;
            // std::cerr << x << " " << std::endl;
            vis[x] = 1;
    ...
    public:
    ISAPSolver(ui _n,ui _look_ahead_prune){
        init_n = _n;
        ...
        tot = timestamp = scc_num = top = dfs_cnt = 0;
    ...
    int run(ui *rid, ui *net_loss, ui *color_buf_1, ui *color_buf_2, ui &reduce_val, ui K){
        ...
        if(look_ahead_prune){
            ...
            for(ui i = 0;i < n1;++i){
                net_loss[i] = 0;
                for(ui j = head[i];j;j = nex[j]){
                    if(ver[j] != D && e[j] == 1 && id[ver[j]] == id[i] && h[i] + cost[j] == h[ver[j]]){
                        net_loss[edge_id[j]] = dep[ver[j]] + h[T] - h[ver[j]] + h[i] - h[D] + cost[j];
                        rid[edge_id[j]] = 2;
                    }
                    else if(ver[j] != D && e[j] == 1){
                        net_loss[edge_id[j]] = dep[ver[j]] + h[T] - h[ver[j]] + h[i] - h[D] + cost[j];
                        rid[edge_id[j]] = 0;
                    }
                    else if(ver[j] != D && e[j] == 0 && h[i] + cost[j] != h[ver[j]]){
                        rid[edge_id[j]] = 3;
                    }
                    else if(ver[j] == D && id[i] == id[ver[j]] && h[i] + cost[j] == h[ver[j]]){
                        color_buf_1[i] = 1;
                    }
                }
            }
            ...
            for(ui i = n1;i < n1+n2; ++i)
                for(ui j = head[i];j;j = nex[j])
                    if(ver[j] == T && id[i] == id[ver[j]] && h[i] + cost[j] == h[ver[j]]){
                        color_buf_2[i-n1] = 1;
                    }
        }
